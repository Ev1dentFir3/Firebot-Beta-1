...
if (pluginListLength != 0) {
  for (i = 0; i < pluginListLength; i++) {
    ...
    try {
      ...
      //don't forget to remove the unloaded plugin from the file aray!!!
    }
  }
}
...
client.on("message", message => {
  ...
  for (i = 0; i < pluginMessage.length; i++) {
    try {
      ...
      console.log('There was an error while exicuting event ' + pluginMessage[i] + 'Error: ' + err)
      message.channel.send("There was an internal error with one of the bot plugins, please let your admin know to check the console.")
    }
  }
});
...
client.on("ready", () => {
  ...
  client.channels.find("name", "general").send('Firebot V1.0 has started!')
  ...
  for (i = 0; i < pluginReady.length; i++) {
    try {
      ...
      console.error('There was an error while exicuting event ' + pluginReady[i] + 'Error: ' + err)
    }
  }
});
